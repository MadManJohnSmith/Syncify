...
# Use a clean absolute path without '..' to avoid Errno 22 on Windows
SPOTIFY_CACHE_PATH = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), '.spotify_token_cache.json')
...
def get_spotify_connection(scopes="user-library-read", verbose_flag=False, project_root_dir=None):
    ...
    try:
        ...
        # DEBUG LOGGING
        try:
            with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '.gui_auth_debug.log'), "a") as f:
                f.write(f"{time.strftime('%Y-%m-%d %H:%M:%S')} - [API] get_spotify_connection called. Scopes: {scopes_str}\n")
                f.write(f"{time.strftime('%Y-%m-%d %H:%M:%S')} - [API] Client ID present: {bool(client_id)}\n")
                f.write(f"{time.strftime('%Y-%m-%d %H:%M:%S')} - [API] Redirect URI: {redirect_uri}\n")
                f.write(f"{time.strftime('%Y-%m-%d %H:%M:%S')} - [API] Cache path: {SPOTIFY_CACHE_PATH}\n")
        except: pass
