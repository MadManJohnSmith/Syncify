//! S179 ARTISTS_TAGS Roundtrip Test Suite
//!
//! Validates:
//! 1. Writing multiple Vorbis comments `ARTISTS_TAGS=<value>` in FLAC.
//! 2. Roundtrip persistence and verification via `apply_and_verify_flac_tags`.
//! 3. Writing and verification in MP4/M4A via `apply_and_verify_mp4_tags`.
...
#[test]
fn test_flac_artists_tags_multi_comment_roundtrip() {
    let dir = tempdir().expect("tempdir");
    let file_path = dir.path().join("artist_tags_track.flac");
    create_synthetic_flac(&file_path);

    let meta = FlacMetadata {
        title: "Paper Trails".to_string(),
        artist: "Darkside".to_string(),
        album: "Psychic".to_string(),
        track_number: 1,
        artist_tags: Some(vec![
