...
from services.streamrip_downloader import download_urls
from services.lyric_fetcher import fetch_lyrics
from services.qobuz_migrator import QobuzMigrator
...
async def run_sync_process(project_root_dir): 
    ...
    # Sync command
    sync_parser = subparsers.add_parser("sync", help="Sync your local library with Spotify.")
    sync_parser.add_argument("music_directories", nargs='+', help="Path(s) to your local music director(y/ies).")
    sync_parser.add_argument("--threshold", type=int, default=APP_CONFIG["default_similarity_threshold"],
                        help=f"Similarity threshold (0-100, config default: {APP_CONFIG['default_similarity_threshold']})")
    sync_parser.add_argument("--review-threshold", type=int, default=APP_CONFIG["default_review_threshold"],
    ...
    sync_parser.add_argument("--session-file", type=str,
    ...
    sync_parser.add_argument("--force-rescan", action="store_true", help="Force rescan, ignoring session file.")
    sync_parser.add_argument("--no-save-session", action="store_true", help="Disable saving session data.")
    sync_parser.add_argument("--dry-run", action="store_true", help="Perform a dry run; no changes made to Spotify.")
    sync_parser.add_argument("--process-orphans", choices=['display', 'add-to-liked', 'add-to-playlist'],
                        const='display', nargs='?', 
                        help="Process local tracks not in Spotify Liked Songs. Actions: 'display' (default), 'add-to-liked', 'add-to-playlist'.")
    sync_parser.add_argument("--orphan-playlist-name", type=str,
    ...
    # Download command
    download_parser = subparsers.add_parser("download", help="Download music from various services.")
    download_parser.add_argument("urls", nargs='+', help="URLs to download.")

    # Fetch lyrics command
    lyrics_parser = subparsers.add_parser("fetch-lyrics", help="Fetch lyrics for a track.")
    lyrics_parser.add_argument("track", help="The name of the track.")
    lyrics_parser.add_argument("artist", help="The name of the artist.")
    lyrics_parser.add_argument("album", help="The name of the album.")

    # Migrate Qobuz command
    migrate_parser = subparsers.add_parser("migrate-qobuz", help="Migrate Qobuz favorites.")
    migrate_parser.add_argument("from_email", help="The email of the account to migrate from.")
    migrate_parser.add_argument("from_pass", help="The password of the account to migrate from.")
    migrate_parser.add_argument("to_email", help="The email of the account to migrate to.")
    migrate_parser.add_argument("to_pass", help="The password of the account to migrate to.")

    args = parser.parse_args()

    # Setup logging now that args.verbose is known
    setup_logging(project_root_dir, getattr(args, "verbose", False))

    if args.command == "download":
        await download_urls(args.urls)
        return

    if args.command == "fetch-lyrics":
        lyrics = fetch_lyrics(args.track, args.artist, args.album)
        if lyrics:
            console.print(lyrics)
        else:
            console.print("[yellow]Lyrics not found.[/yellow]")
        return

    if args.command == "migrate-qobuz":
        app_id = os.getenv("QOBUZ_APP_ID")
        app_secret = os.getenv("QOBUZ_APP_SECRET")
        if not app_id or not app_secret:
            console.print("[red]Error: QOBUZ_APP_ID and QOBUZ_APP_SECRET environment variables not set.[/red]")
            return
        from_creds = {"email": args.from_email, "password": args.from_pass}
        to_creds = {"email": args.to_email, "password": args.to_pass}
    ...
    if args.command == "sync":
        ...
        if not local_music_paths:
            console.print("[red]Error: No music directories provided.[/red]")
            return
