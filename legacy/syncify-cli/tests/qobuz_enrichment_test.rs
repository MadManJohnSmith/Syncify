use syncify_tauri_lib::download::{apply_flac_tags, download_goodies_booklet, FlacMetadata};
...
async fn test_goodies_booklet_download_parsing() {
    ...
    let result: Option<std::path::PathBuf> = download_goodies_booklet(&client, "", &temp_dir).await;
...
async fn test_sqlite_manual_source_type_protection() {
    ...
    let meta = syncify_tauri_lib::services::enrichment::EnrichedMetadata {
...
async fn test_matrix_track10_prodigy_firestarter() {
    ...
    let flac_meta = FlacMetadata {
        ...
        bpm: enriched.bpm.map(|b| b.round() as u32),
...
async fn test_real_qobuz_download_heroes() {
    ...
    let downloader = syncify_tauri_lib::download::QobuzDownloader::new();
    let request = syncify_tauri_lib::download::DownloadRequest {
    ...
    let mut flac_meta = syncify_tauri_lib::download::build_flac_metadata(&download_result, &request);
    ...
    flac_meta.bpm = enriched.bpm.map(|b| b.round() as u32);
