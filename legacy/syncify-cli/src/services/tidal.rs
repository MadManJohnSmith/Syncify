...
/// Refresh an expired Tidal access token using stored `refresh_token`
pub async fn refresh_gui_token(
    client: &reqwest::Client,
    refresh_token: &str,
) -> Result<(String, TidalGuiCredentials), String> {
    let client_id = "fX2JxdmntZWK0ixT";
    let client_secret = "xeuPmY7nbpZ9IIbLAcQ93shka1VNheUAqN6IcszjTG8=";
    ...
    let updated_creds = TidalGuiCredentials {
        ...
        user_id: json_val.get("user").and_then(|u| u.get("userId")).cloned(),
        country_code: json_val.get("user").and_then(|u| u.get("countryCode")).and_then(|v| v.as_str()).map(|s| s.to_string()),
    };
...
pub async fn resolve_gui_credentials_from_pool(
    ...
    if let Some(ref ref_tok) = creds.refresh_token {
        if !ref_tok.trim().is_empty() {
            match refresh_gui_token(http_client, ref_tok).await {
                Ok((new_access_token, updated_creds)) => {
                    // Re-encrypt updated credentials JSON
                ...
                Err(e) => {
                    return (None, TidalAuthResolution::SourceUnavailable(format!("GUI token refresh failed: {}", e)));
                }
            }
        }
    }
