                ...
                match outcome {
                    ...
                    Err(err_msg) => {
                        ...
                        summary.manifest.push(TrackManifestEntry {
                            ...
                            format_requested: quality_opt.unwrap_or("24-192").to_string(),
...
async fn download_track_item(
    ...
    if output_file_path.exists() && !force_overwrite {
        ...
        return Ok(TrackManifestEntry {
            ...
            format_requested: requested_fmt_id.to_string(),
            format_obtained: None,
            quality_class_requested: "Lossless".to_string(),
            quality_class_obtained: None,
            codec: Some(if is_mp3 { "MP3".to_string() } else { "FLAC".to_string() }),
            container: Some(if is_mp3 { "MP3".to_string() } else { "FLAC".to_string() }),
            ...
            size_bytes: size,
            flac_validation: if is_mp3 { "None".to_string() } else { "Valid".to_string() },
            tagging_result: "Skipped".to_string(),
            enrichment_result: "Skipped".to_string(),
    ...
    Ok(TrackManifestEntry {
        ...
        album: album.to_string(),
        format_requested: requested_fmt_id.to_string(),
        format_obtained: Some(target_format_id.to_string()),
        quality_class_requested: if requested_fmt_id == "5" { "Lossy".to_string() } else { "Lossless".to_string() },
        quality_class_obtained: Some(if is_mp3 { "Lossy".to_string() } else { "Lossless".to_string() }),
        codec: Some(if is_mp3 { "MP3".to_string() } else { "FLAC".to_string() }),
        container: Some(if is_mp3 { "MP3".to_string() } else { "FLAC".to_string() }),
        extension: Some(ext.to_string()),
        source: Some("Qobuz API".to_string()),
        quality_fallback: false,
        download_result: "Success".to_string(),
        rejection_reason: None,
        audio_validation: "Valid".to_string(),
